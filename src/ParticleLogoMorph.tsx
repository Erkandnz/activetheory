...
// Efekt penceresinin dışında parçacıklar TAMAMEN görünmez olur (opacity 0).
// Bu değer, görünür/görünmez arası ne kadar yumuşak geçileceğini belirler
// (sayfa scroll yüzdesi cinsinden). Aksi halde parçacıklar sitenin her
// yerinde sürekli görünür kalır.
const FADE_MARGIN = 0.06;
...
export default function ParticleLogoMorph() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Sadece [TRIGGER_START - FADE_MARGIN, TRIGGER_END + FADE_MARGIN]
      // aralığında görünür; dışında tamamen saydam (görünmez).
      const fadeIn = smoothstep(TRIGGER_START - FADE_MARGIN, TRIGGER_START, p);
      const fadeOut = 1 - smoothstep(TRIGGER_END, TRIGGER_END + FADE_MARGIN, p);
      targetVisibility = Math.min(fadeIn, fadeOut);
    };
    ...
    const animate = () => {
      ...
      visibility += (targetVisibility - visibility) * Math.min(1, dt * 4);

      material.opacity = BASE_OPACITY * visibility;
      points.visible = visibility > 0.001;

      if (!points.visible) {
        // Pencerenin tamamen dışındayız: gereksiz hesap/render yapma.
